    ...
    while (cur) {
      for (uint32_t j = 0; j < cur->rebindings_nel; j++) {
        ...
        if (strcmp(&symbol_name[symbol_name_offset], cur->rebindings[j].name) == 0) {
          ...
          vm_address_t page_start = (vm_address_t)&indirect_symbol_bindings[i] & ~(vm_page_size - 1);
          vm_protect(mach_task_self(), page_start, vm_page_size, false, VM_PROT_READ | VM_PROT_WRITE | VM_PROT_COPY);
